...
export var SN_DD_INTRO = 'SharkNinja still has no Summit DCF model, but a Bloomberg (BST) company-financials export landed Sep 2026 — Evolution ▸ Results carries real Street consensus (not just company guidance), and Bottom Line is built from it. Top Line, Management and Miscellaneous are hand-authored from the 10-K, proxy statement and earnings releases. Estimates (the vintage/snapshot view) and Earnings Setup are still blocked — that file is a single current snapshot, not an archive of how consensus moved over time — and Valuation is not built yet.';
