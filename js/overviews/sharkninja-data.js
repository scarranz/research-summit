...
export var SN_TIMELINE = [
  ...
  // The four entries below came in from the Deep Dive's old Evolution ▸ Timeline (Sep 2026), which was
  // retired to match Amazon's Evolution spine. Only the events that pass OVERVIEW_CONVENTIONS §4.7 moved
  // here; the print-by-print record (calls, guidance raises, CFO changes, the insider sale) went to
  // Management ▸ Track Record instead, so nothing was deleted.
  ['2024–2026', 'New categories become the growth engine — <b>Beauty &amp; Home Environment</b> grows +45.3% in FY2025 to ~$826M, about 13% of net sales, led by launches outside the original vacuum/blender core.', ['2024 — Ninja <b>Crispi</b>, the first portable glass air fryer; it becomes a platform (Crispi Pro, DualZone Crispi) rather than a single product.', '2025 — Ninja <b>SLUSHi</b> (frozen drinks), Shark <b>CryoGlow</b> face masks (the company\'s first skincare entry) and the Ninja <b>Luxe Café</b> espresso machine; Food Preparation +31.6% for the year.', 'Jul 2026 — the Ninja <b>Crispi Microwave</b>, the 40th sub-category and a multi-billion-dollar category the company had never touched; a 41st was flagged for late 3Q26.', 'Scale check (FY2025): Cleaning ~$2.2B, Cooking &amp; Beverage ~$1.8B, Food Preparation ~$1.6B, Beauty &amp; Home Environment ~$0.8B — Beauty is the fastest-growing category and still the smallest.']],
  ['Jul 31, 2025', 'The last operating tie to the former parent ends: the <b>JS Global sourcing-service fee</b> terminates.', ['A contractual fee paid to JS Global for supply-chain services, a leftover of the pre-spin structure.', 'The company names its end as a gross-margin tailwind in every release since — a structural improvement, not an operating one.']],
  ['Feb 11, 2026', 'Board authorizes a <b>$750M share-repurchase program</b> — the company\'s first capital-return program as an independent entity.', ['Authorized alongside the FY2025 results, on record cash.', 'The company states it does not expect to incur debt to fund it.']],
  ['2Q26', 'International goes <b>direct</b>: Italy and Spain finish converting from distributors to SharkNinja\'s own operation — the last of them.', ['The model the UK proved in 2014; Mexico converted a year earlier.', 'The Salesforce DTC platform finished rolling out across the major international markets in the same quarter.', 'Management: "now done for the foreseeable future with these distributor conversions." International grew +36.6% in 2Q26.']],
];
...
export var SN_DD_INTRO = 'SharkNinja still has no Summit DCF model, but a Bloomberg (BST) company-financials export landed Sep 2026 — Evolution ▸ Results carries real Street consensus (not just company guidance), and Bottom Line is built from it. Top Line is engine-driven from the segments dataset; Management and Miscellaneous are hand-authored from the 10-K, proxy and earnings releases. Two Evolution sub-tabs stay deliberately unbuilt, and the reason is the same for both: the vintage machinery reads snapshots keyed by date out of BBG_CONSENSUS.txt, and SN is not one of the eight tickers in that file. No consensus vintages means no Estimates pane, and no authorized KPI set means no Earnings Setup grid (Earnings conventions §5 rule 6). Neither gap is fillable from Quartr, which carries primary filings and transcripts but no analyst estimates. Guidance covers what IS sourceable — the company\'s own FY outlook, print by print. Valuation ships Peers in full and Historic Multiple partially — its multiple history needs the undeployed get-market-history edge function plus the same consensus vintages, so the pane carries the current point instead; Target Multiple / PEG and Sensitivity Analysis are hand-transcribed from Summit DCF snapshots and there is no model for SN.';
...
export var SN_TL_CATEGORY_GROWTH = 'FY2025 net sales grew across all four product categories. <b>Beauty and Home Environment</b> led at <b>+45.3%</b>, driven by fans/air purifiers and a new face-mask category launched in 2025. <b>Food Preparation</b> grew <b>+31.6%</b> on the frozen-drinks sub-category (SLUSHi) and ice-cream makers. <b>Cooking and Beverage</b> (<b>+5.7%</b>) grew on the <b>Ninja Luxe Café</b> espresso machine, and <b>Cleaning</b> grew <b>+6.9%</b>. The 10-K does not break out category revenue in dollars — but <b>every quarterly earnings release does</b>, and that full series is now under <b>Top Line &#9656; Categories</b>. (Earlier drafts of this note attributed the +31.6% to the frozen-drinks sub-category alone; it is the Food Preparation category total, corrected against the releases.)';
...
export var SN_BL_DEBT_FLAG = 'This does not cleanly reconcile with Miscellaneous ▸ Other Analysis, which cites the company\'s own FY2025 press release showing Total Debt of $736.1M against $777.3M cash — implying a small NET CASH position (~$41M), not net debt. RESOLVED (Quartr pass, Sep 2026): the company publishes its own capital-structure summary, and at Jun 30, 2026 it reports cash $780M, total debt $719M, NET CASH of $61M and net leverage of (0.05x) — confirming that on the company\'s own definition (cash less term-loan borrowings) SharkNinja carries no net debt. The gap versus Bloomberg is definitional, not a disputed fact; operating lease liabilities under ASC 842 remain the likely but still unconfirmed explanation. See Miscellaneous ▸ Tariffs & Capital Structure.';
...
export var SN_MGMT_TRACK_NOTE = 'Guidance hit-rate against the company\'s own outlook is covered by the guidance walk under Management ▸ Track Record and by Evolution ▸ Results, not repeated here.';
