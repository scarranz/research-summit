// team.js — Investment Portfolio + Investment Ideas
// NOTE FOR FUTURE DEVELOPMENT:
// ─────────────────────────────────────────────────────────────
// This module currently stores Investment Ideas data IN-MEMORY only.
// All sessions, votes, and companies reset on page refresh.
// The Portfolio table uses localStorage.
//
// NEXT STEP: Migrate both features to Supabase persistence.
// When building this out:
// 1. Create tables: `idea_sessions`, `idea_companies`, `idea_votes`
//    in Supabase (see sql/ for patterns)
// 2. Add CRUD functions to js/api.js (fetchSessions, insertVote, etc.)
// 3. Replace the II_SESSIONS array with API calls
// 4. Replace localStorage portfolio with a `portfolio_scores` table
// 5. Follow the same data access pattern as company_resources:
//    api.js handles all DB queries, this module only calls api.js
// ─────────────────────────────────────────────────────────────
...
// Seed data — will be replaced with Supabase queries
const II_SESSIONS = [
  {
...
function iiRenderSb() { /* Scoreboard — not yet implemented */ }
...
window.iiOpenNewSession = function () {
  ...
  document.getElementById('ii-newSessionModal').classList.add('open');
};
...
window.iiOpenEditSession = function (i) {
  ...
  let dateVal = '';
  if (parts.length === 3) {
    const m = String((months[parts[0].slice(0,3)] ?? 0) + 1).padStart(2,'0');
...
window.iiDeleteSession = function (i) {
  if (II_SESSIONS.length === 1) return;
...
window.iiOpenNewCompany = function () {
  ...
  document.getElementById('ii-newCompanyModal').classList.add('open');
};
    ...
    if (isExp) {
      ...
      detailHtml = `<div class="ii-co-detail" onclick="event.stopPropagation()">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:4px">
          ...
          <button class="ii-btn-sm" onclick="event.stopPropagation();iiCollapseCard()">Close</button>
        </div>
      </div>`;
    }
      ...
      <div class="ii-co-header">
        <div>
          <div style="display:flex;align-items:center;gap:5px;flex-wrap:wrap">
            ...
            ${pending.length && !isExp ? `<span class="ii-pending-badge">${pending.length} pending</span>` : ''}
          </div>
