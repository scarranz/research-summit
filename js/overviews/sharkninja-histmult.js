// overviews/sharkninja-histmult.js — SharkNinja (SN) Deep Dive ▸ Valuation ▸ Historic Multiple,
// on AMAZON'S machinery.
//
// WHY THIS FILE IS A COPY (Sep 2026). SAB wants SN's Valuation to look and behave exactly like
// Amazon's, and a re-implementation drifts (the Earnings tab proved it three times). So this is
// js/overviews/amzn-histmult.js copied verbatim — same functions, markup, class names, inline CSS,
// controls, chart kit and interactions — with ONLY the data layer swapped. Every change, so a diff
// against amzn-histmult.js is not a surprise:
//   • import snResults (results-data/sn.js) instead of amznResults; export name snHistMult.
//   • ticker 'AMZN' → 'SN' in fetchPriceHistory / fetchRatiosHistory (same get-market-history path,
//     same "live data unavailable" state while the function is undeployed).
//   • START '2022-10-03' → '2023-07-31': SN began trading on the NYSE on Jul 31, 2023 (spin-off
//     from JS Global); there is no price before it.
//   • REPORT DATES. AMZN reads print dates off estMatrix.cons vintages. SN has NO vintage archive,
//     so reportDateForQ() reads a HAND-KEPT calendar (SN_REPORT_DATES, from the Quartr event
//     record, 3Q23 Nov 9 2023 … 2Q26 Aug 5 2026). FY2022's "known on" date is the listing date
//     (its audited figures were in the spin-off filings before SN traded).
//   • FORWARD denominators. AMZN steps through many Bloomberg vintages. SN has ONE snapshot — the
//     Sep 2026 Bloomberg (BST) export in results-data/sn.js (annual `cons`, FY2026–FY2029). It is
//     treated as ONE vintage dated SN_CONS_VINTAGE = '2026-09-01' and is NOT back-filled across
//     history (that would be look-ahead: pricing 2024 against a 2026 estimate). So every forward
//     line exists only from Sep 1, 2026; before that the module's own "no estimate on this date"
//     gap and the range readout's "no estimate this far back" state render. Said in the footer.
//   • EPS basis is ADJUSTED diluted EPS (epsAdj — what SN headlines and guides); EBITDA basis is
//     ADJUSTED EBITDA (ebitdaAdj). Forward FY EPS reads the annual cons row directly (AMZN had to
//     sum four quarters because its annual Bloomberg EPS is basis-flagged; SN's is not).
//   • TRAILING. LTM EPS = sum of four quarters of views.q.metrics.epsAdj.act. Unlike Amazon, SN HAS
//     quarterly adjusted EBITDA actuals (ebitdaAdj.act), so LTM EV/EBITDA is WIRED (AMZN left
//     ACT.ebitda empty). The quarterly series starts at 3Q23, so the first LTM point is the 2Q24
//     print (Aug 8, 2024); Last FY covers the stretch before it from the annual actuals.
//   • Last-FY values read the annual act rows (epsAdj / ebitdaAdj) directly — SN's quarterly
//     series starts at 3Q23, so summing quarters would lose FY2022–FY2023.
//   • numerAt/buildSeries return a gap (null) when shares or net debt are not yet on file, instead
//     of Amazon's px × null + null = 0 → a fabricated 0× EV/EBITDA. SN's ratios history starts
//     later than AMZN's, so that edge is reachable here.
//   • EBITDA denominator formatting: '$1.36B' (two decimals) instead of '$1B' — SN's EBITDA is ~$1B,
//     so Amazon's whole-billion rounding would print the same number for every year.
//   • Footer sourcing paragraph rewritten to SN's truthful sources and caveats (single snapshot,
//     hand-kept calendar, adjusted bases, LTM EV/EBITDA available). Structure unchanged.
//   • Header/data comments below edited where they described Amazon's data; design commentary kept.
//
// ── AMAZON'S ORIGINAL DESIGN NOTES (kept) ─────────────────────────────────────────
...
//   EST       Street adj. EPS / adj. EBITDA per fiscal year → views.y.metrics.epsAdj.cons and
//             ebitdaAdj.cons (js/results-data/sn.js), the ONE Bloomberg (BST) Sep 2026 snapshot,
//             dated SN_CONS_VINTAGE. No revision history exists for SN, so each fiscal year is a
//             one-step series starting on that date — never back-filled.
//   ACT       LTM adj. EPS / adj. EBITDA at each print → trailing four quarters of
//             views.q.metrics.epsAdj.act and ebitdaAdj.act. Last FY → the annual act rows.
//
// Every print date used for STEPPING (when a denominator changes) comes from SN_REPORT_DATES,
// a hand-kept calendar from the Quartr event record — acceptable only because SN has no vintage
// archive to read them from (Amazon's copy reads them off estMatrix.cons.vintages).
//
...
function todayStr(){ var d=new Date();
  return d.getUTCFullYear()+'-'+('0'+(d.getUTCMonth()+1)).slice(-2)+'-'+('0'+d.getUTCDate()).slice(-2); }
var START = '2023-07-31', TODAY = todayStr();        // SN's first NYSE trading day

// Fiscal years are calendar years for SN.
...
// ── Real data, built from js/results-data/sn.js and the get-market-history edge function ──────
...
// SN has no estMatrix vintage archive, so report dates are a HAND-KEPT calendar — from the Quartr
// event record (earnings-release dates). Amazon's copy reads these off its vintages instead.
var SN_REPORT_DATES = {
  '3Q23':'2023-11-09', '4Q23':'2024-02-15', '1Q24':'2024-05-09', '2Q24':'2024-08-08',
  '3Q24':'2024-10-31', '4Q24':'2025-02-13', '1Q25':'2025-05-08', '2Q25':'2025-08-07',
  '3Q25':'2025-11-06', '4Q25':'2026-02-11', '1Q26':'2026-05-06', '2Q26':'2026-08-05',
};
// When each fiscal year's full-year actual became known: its 4Q print. FY2022 was already in the
// spin-off filings before SN listed, so it is known from the first trading day (START).
var SN_FY_KNOWN = { '2022':START, '2023':'2024-02-15', '2024':'2025-02-13', '2025':'2026-02-11' };
// The ONE Bloomberg (BST) consensus snapshot in results-data/sn.js ('Sep 2026'), treated as a
// single vintage on this date. Forward lines exist only from here — no back-fill (look-ahead).
var SN_CONS_VINTAGE = '2026-09-01';

function reportDateForQ(qLabel){
  return SN_REPORT_DATES[qLabel] || null;
}
// EST[metric][fy] — the annual cons row of the given dataset key, one step on SN_CONS_VINTAGE.
function buildEstFromSnapshot(key){
  var m=snResults.views.y.metrics[key], out={};
  m.periods.forEach(function(fy,i){
    var val=num(m.cons[i]); if(val==null) return;
    out[fy]=[[SN_CONS_VINTAGE, val]];
  });
  return out;
}
// ACT[metric] — real trailing-4-quarter sum of views.q.metrics[key].act, stepped at each print.
function buildActLtm(key){
  var m=snResults.views.q.metrics[key], periods=m.periods, act=m.act, out=[];
...
// ACT_FY — the last fiscal year fully reported, stepped at the date it became known.
function buildActFy(){
  return Object.keys(SN_FY_KNOWN).sort().map(function(y){ return [SN_FY_KNOWN[y], +y]; });
}
// ACT_FY_V[metric] — real annual actuals, direct from the dataset's annual act row.
function buildActFyFromAnnual(key){
  var m=snResults.views.y.metrics[key], out={};
...
// millions of shares (see results-data/sn.js's own header), so both get ÷1e6 here, once, at the
...
function hmLoad(){
  ...
  return Promise.all([
    ...
    EST.eps = buildEstFromSnapshot('epsAdj');
    ACT.eps = buildActLtm('epsAdj');
    ACT.ebitda = buildActLtm('ebitdaAdj');   // SN reports quarterly adj. EBITDA — AMZN left this a gap
    ACT_FY = buildActFy();
...
function numerAt(metric, d, px){
  ...
  var sh=stepAt(SHARES, d), nd=stepAt(NETDEBT, d);
  if(sh==null || nd==null) return null;                           // SN: a gap, never px × null = 0
  return (px * sh) + nd;                                          // $M: EV
}
...
var METRICS = {
  ...
  evebitda: { label:'EV/EBITDA',  metric:'ebitda', denomName:'EBITDA',
              denomFmt:function(v){ return '$'+(v/1000).toFixed(2)+'B'; } },
};
...
function buildSeries(mk, hz){
  ...
  PX.forEach(function(p){
    ...
    out.push({ d:p.d, px:p.px, den:den, v:(den==null || den<=0 || num==null)?null:(num/den) });
  });
  return out;
}
...
export var snHistMult = { body: hmBody, init: initHm };
